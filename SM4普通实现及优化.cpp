...
void SIMD_SM4(u32 X[], u32 RK[], u32 Y[],int enc) {
	__m256i XX[4], Temp[4], Mask;
	Mask = _mm256_set1_epi32(0xFF);
	Temp[0] = _mm256_loadu_si256((const __m256i*)X + 0);    //加载数据
	Temp[1] = _mm256_loadu_si256((const __m256i*)X + 1);
	Temp[2] = _mm256_loadu_si256((const __m256i*)X + 2);
	Temp[3] = _mm256_loadu_si256((const __m256i*)X + 3);
	XX[0] = MM256_PACK0_EPI32(Temp[0], Temp[1], Temp[2], Temp[3] );    //合并每组128bit数据的某32bit字
	XX[1] = MM256_PACK1_EPI32(Temp[0], Temp[1], Temp[2], Temp[3] );
	XX[2] = MM256_PACK2_EPI32(Temp[0], Temp[1], Temp[2], Temp[3] );
	XX[3] = MM256_PACK3_EPI32(Temp[0], Temp[1], Temp[2], Temp[3] );
	__m256i vindex = _mm256_setr_epi8(3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12, 3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12);//转化端序
	XX[0] = _mm256_shuffle_epi8(XX[0], vindex);
	XX[1] = _mm256_shuffle_epi8(XX[1], vindex);
	XX[2] = _mm256_shuffle_epi8(XX[2], vindex);
	XX[3] = _mm256_shuffle_epi8(XX[3], vindex);
	for (int i = 0;i < 32;i++) {   //32轮迭代
		__m256i k = _mm256_set1_epi32((enc == 0) ? RK[i] : RK[31 - i]);
		Temp[0] = _mm256_xor_si256(_mm256_xor_si256(XX[1], XX[2]), _mm256_xor_si256(XX[3], k));
		Temp[1] = _mm256_xor_si256(XX[0], _mm256_i32gather_epi32((const int*)BOX0, _mm256_and_si256(Temp[0], Mask), 4));   //查表
		Temp[0] = _mm256_srli_epi32(Temp[0], 8);
		Temp[1] = _mm256_xor_si256(Temp[1], _mm256_i32gather_epi32((const int*)BOX1, _mm256_and_si256(Temp[0], Mask), 4));
		Temp[0] = _mm256_srli_epi32(Temp[0], 8);
		Temp[1] = _mm256_xor_si256(Temp[1], _mm256_i32gather_epi32((const int*)BOX2, _mm256_and_si256(Temp[0], Mask), 4));
		Temp[0] = _mm256_srli_epi32(Temp[0], 8);
		Temp[1] = _mm256_xor_si256(Temp[1], _mm256_i32gather_epi32((const int*)BOX3, _mm256_and_si256(Temp[0], Mask), 4));
		XX[0] = XX[1];
		XX[1] = XX[2];
		XX[2] = XX[3];
		XX[3] = Temp[1];
	}
	_mm256_storeu_si256((__m256i*)Y + 0, MM256_PACK0_EPI32(XX[3], XX[2], XX[1], XX[0]));  //恢复分组并填充
	_mm256_storeu_si256((__m256i*)Y + 1, MM256_PACK0_EPI32(XX[3], XX[2], XX[1], XX[0]));
	_mm256_storeu_si256((__m256i*)Y + 2, MM256_PACK0_EPI32(XX[3], XX[2], XX[1], XX[0]));
	_mm256_storeu_si256((__m256i*)Y + 3, MM256_PACK0_EPI32(XX[3], XX[2], XX[1], XX[0]));
}

void encryptSM42(u32 X[], u32 RK[], u32 Y[]) {
	SIMD_SM4(X, RK, Y, 0);
}


void decryptSM42(u32 X[], u32 RK[], u32 Y[]) {
	SIMD_SM4(X, RK, Y, 1);
}
...
int main(void) {
	_LARGE_INTEGER t1;  //开始时间1
	_LARGE_INTEGER t2;   //结束时间1
	_LARGE_INTEGER t3;  //开始时间2
	_LARGE_INTEGER t4;   //结束时间2
	_LARGE_INTEGER t5;  //开始时间3
	_LARGE_INTEGER t6;   //结束时间3
	_LARGE_INTEGER t7;  //开始时间4
	_LARGE_INTEGER t8;   //结束时间4
	double dqFreq;      //计时器频率
	...
	double rt1 = 1000000 * (t2.QuadPart - t1.QuadPart) / dqFreq; //乘以1000000把单位由秒化为微秒，精度为1000 000/（cpu主频）微秒
	printf("\n基础加密时间：%fus\n", rt1);
	//printf("%08x %08x %08x %08x\n", Y[0], Y[1], Y[2], Y[3]);
	printf("************** 基础生成明文 *****************\n");
	QueryPerformanceCounter(&t3);   //计时开始
	decryptSM4(Y, RK, X);
	QueryPerformanceCounter(&t4);    //计时结束
	double rt2 = 1000000 * (t4.QuadPart - t3.QuadPart) / dqFreq; //乘以1000000把单位由秒化为微秒，精度为1000 000/（cpu主频）微秒
	printf("\n基础解密时间：%fus\n", rt2);
	//printf("%08x %08x %08x %08x\n", X[0], X[1], X[2], X[3]);
	printf("************** SIMD生成密文 *****************\n");
	QueryPerformanceCounter(&t5);   //计时开始
	encryptSM42(X, RK, Y);
	QueryPerformanceCounter(&t6);    //计时结束
	double rt3 = 1000000 * (t6.QuadPart - t5.QuadPart) / dqFreq; //乘以1000000把单位由秒化为微秒，精度为1000 000/（cpu主频）微秒
	printf("\nSIMD加密时间：%fus\n", rt3);
	//printf("%08x %08x %08x %08x\n", Y[0], Y[1], Y[2], Y[3]);
	printf("************** SIMD生成明文 *****************\n");
	QueryPerformanceCounter(&t7);   //计时开始
	decryptSM42(Y, RK, X);
	QueryPerformanceCounter(&t8);    //计时结束
	double rt4 = 1000000 * (t8.QuadPart - t7.QuadPart) / dqFreq; //乘以1000000把单位由秒化为微秒，精度为1000 000/（cpu主频）微秒
	printf("\nSIMD解密时间：%fus\n", rt4);
	//printf("%08x %08x %08x %08x\n", X[0], X[1], X[2], X[3]);
	return 0;
}
...
//dd63e022 90fa7d96 5215d84a 6b2f7e78
